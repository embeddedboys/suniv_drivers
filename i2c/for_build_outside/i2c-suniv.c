...
struct suniv_i2c {
        ...
        u32                     rc;             /* Suceessed I2C msg count */
...
static inline void suniv_i2c_write(struct suniv_i2c *i2c_dev, u32 reg,
                                   u32 val)
{
...
static int suniv_i2c_of_config(struct suniv_i2c *i2c_dev,
                               struct device *dev)
{
        int rc = 0;
...
static inline void suniv_i2c_hw_init(struct suniv_i2c *i2c_dev)
{
         ...
         */
        suniv_i2c_write(i2c_dev, i2c_dev->reg_offsets.srst, 1);
        ...
        */
        suniv_i2c_write(i2c_dev, i2c_dev->reg_offsets.ccr,
...
static int suniv_i2c_do_msgs(struct suniv_i2c *i2c_dev)
{
        ...
        for (i = 0; i < num; i++) {
                ...
                /* Temporarily disable wait queue method */
                /*
                time_left = wait_event_timeout(i2c_dev->wait_queue, !i2c_dev->sleep,
...
static int suniv_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
        ...
        pr_debug("%s, successed msg count : %d", __func__, i2c_dev->rc);
        return rc;
}
...
static int suniv_i2c_probe(struct platform_device *pdev)
{
        ...
        printk("%s: alloc memory of i2c_dev\n", __func__);
        ...
        /* init ops */
        init_waitqueue_head(&i2c_dev->wait_queue);
        ...
        i2c_dev->adapter.timeout     = msecs_to_jiffies(20);
